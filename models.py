...
def interpolation(noisy, SNR, Number_of_pilot, interp):
    num_samples, H, W = noisy.shape  # Get dataset size dynamically

    # Create a 4D array for real & imaginary parts
    noisy_image = np.zeros((num_samples, H, W, 2))
    noisy_image[:, :, :, 0] = np.real(noisy)
    noisy_image[:, :, :, 1] = np.imag(noisy)

    # Define pilot indices
    pilot_indices = {
        ...
        24:  [14*i for i in range(1, 72,9)] + [6+14*i for i in range(4, 72,9)] + [11+14*i for i in range(1, 72,9)],
        8:   [4+14*i for i in range(5, 72, 18)] + [9+14*i for i in range(8, 72, 18)],
        36:  [14*i for i in range(1, 72,6)] + [6+14*i for i in range(4, 72,6)] + [11+14*i for i in range(1, 72,6)]
    }
    
    if Number_of_pilot not in pilot_indices:
        raise ValueError("Unsupported number of pilots!")

    idx = pilot_indices[Number_of_pilot]
    r, c = np.array(idx) // 14, np.array(idx) % 14  # Convert 1D indices to 2D row-column

    interp_noisy = np.zeros((num_samples, H, W, 2))

    # Vectorized interpolation for all samples
    X, Y = np.meshgrid(np.arange(H), np.arange(W), indexing="ij")  # Create grid

    for i in range(2):  # Iterate over real (0) and imaginary (1) parts
        z = noisy_image[:, r, c, i]  # Get pilot values for all samples
        
        if interp == 'rbf':
            f = [interpolate.Rbf(r, c, z[j], function='gaussian') for j in range(num_samples)]
            interp_noisy[:, :, :, i] = np.array([f[j](X, Y).T for j in range(num_samples)])

        elif interp == 'spline':
            tck = [interpolate.bisplrep(r, c, z[j]) for j in range(num_samples)]
            interp_noisy[:, :, :, i] = np.array([interpolate.bisplev(np.arange(H), np.arange(W), tck[j]) for j in range(num_samples)])

    # Reshape output as required
    interp_noisy = np.concatenate((interp_noisy[:, :, :, 0], interp_noisy[:, :, :, 1]), axis=0).reshape(2 * num_samples, H, W, 1)
